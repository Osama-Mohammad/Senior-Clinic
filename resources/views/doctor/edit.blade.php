    ...
    <div
        ...
        <div class="max-w-4xl w-full bg-white p-8 rounded-2xl shadow-2xl space-y-8 animate-fade-in">
            ...
            <div class="text-center">
                <h2 class="text-3xl font-extrabold text-teal-700">Edit Doctor: {{ $doctor->first_name }}
                    {{ $doctor->last_name }}</h2>
            ...
            <form action="{{ route('doctor.update', $doctor) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <input type="file" name="image" id="image"
                            class="mt-1 block w-full text-sm text-gray-600 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-teal-50 file:text-teal-700 hover:file:bg-teal-100">
                        @error('image')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="md:col-span-2" x-data="availabilityForm()">
                        ...
                        <template x-for="day in days" :key="day">
                            <div
                                class="flex flex-col md:flex-row items-start md:items-center space-y-1 md:space-y-0 md:space-x-4 mb-2">
                                <label class="flex items-center space-x-2">
                                    <input type="checkbox" :value="day"
                                        @change="toggleDay(day, $event.target.checked)" :checked="selected(day)"
                                    ...
                                    <div class="flex flex-col md:flex-row space-y-1 md:space-y-0 md:space-x-2">
                                        ...
                                        <div>
                                            ...
                                            <template x-if="$refs[`from_${day}_error`]">
                                                <p x-html="$refs[`from_${day}_error`].innerHTML"
                                        ...
                                        <div>
                                            ...
                                            <select :name="`availability_schedule[${day}][to]`" x-model="schedule[day].to"
                                                class="rounded-md border-gray-300 text-sm focus:ring-teal-500 focus:border-teal-500">
                                                <option value="">--</option>
                                                @foreach (['08:00', '09:00', '10:00', '11:00', '12:00', '13:00', '14:00', '15:00', '16:00'] as $hour)
                                                    <option value="{{ $hour }}">{{ $hour }}</option>
                                            ...
                                            <template x-if="$refs[`to_${day}_error`]">
                                                <p x-html="$refs[`to_${day}_error`].innerHTML"
                                                    class="text-red-500 text-xs mt-1"></p>
                                            </template>
                                        </div>
                                        ...
                                        <div class="flex items-center space-x-2">
                                            ...
                                            <!-- 💡 Suggested Max (inline) -->
                                            <span class="text-xs text-gray-500" x-show="suggestedMax(day) !== null">
                                                💡 <span x-text="suggestedMax(day)"></span>
                                            </span>
                                        </div>

                                        <template x-if="$refs[`max_${day}_error`]">
                                            <p x-html="$refs[`max_${day}_error`].innerHTML"
                                                class="text-red-500 text-xs mt-1"></p>
                                        </template>
                ...
                <div class="border-t pt-6 mt-8 md:col-span-2">
                    <h3 class="text-lg font-semibold text-teal-700 mb-2">🔒 Update Password</h3>
    ...
    @foreach (['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'] as $day)
        <div class="hidden">
            <span x-ref="from_{{ $day }}_error">
                @error("availability_schedule.$day.from")
                    {{ $message }}
                @enderror
            </span>
            <span x-ref="to_{{ $day }}_error">
                @error("availability_schedule.$day.to")
                    {{ $message }}
                @enderror
            </span>
            <span x-ref="max_{{ $day }}_error">
                @error("availability_schedule.$day.max")
                    {{ $message }}
                @enderror
            </span>
        </div>
    ...
    <script>
        function availabilityForm() {
            return {
                ...
                alertIfTooHigh(day) {
                    const suggested = this.suggestedMax(day);
