<x-layout>
    <form action="{{ route('patient.store') }}" method="POST">
        ...
        <select name="address" id="address">
            <option value="" disabled {{ old('address', $patient->address ?? '') ? '' : 'selected' }}>Select a City
            </option>
            @foreach ($cities as $city)
                <option value="{{ $city }}" {{ old('address', $patient->address ?? '') == $city ? 'selected' : '' }}>
                    {{ $city }}
                </option>
            @endforeach
        </select>
        ...
        <select name="gender" id="gender">
            <option value="" disabled {{ old('gender', $patient->gender ?? '') ? '' : 'selected' }}>Select a Gender
            </option>
