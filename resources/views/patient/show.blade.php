<x-layout>
    ...
    <section id="top" x-data="{
        ...
        <div class="relative z-20 flex items-center h-full px-6 md:px-20">
            <div class="text-white max-w-xl space-y-6">
                <p class="text-lg text-teal-200 font-medium tracking-wide">Trusted by thousands of patients</p>
                <h1 class="text-4xl sm:text-5xl lg:text-6xl font-bold leading-tight">LEADING HEALTHCARE<br class="hidden sm:block"> SOLUTIONS</h1>
                <p class="text-base sm:text-lg text-gray-200">We deliver comprehensive services with compassion.</p>
                <a href="#clinics" class="inline-block bg-teal-500 hover:bg-teal-600 text-white font-semibold px-6 py-3 rounded-md transition">Explore Clinics</a>
            </div>
        </div>
        <button @click="current = (current === 0) ? images.length - 1 : current - 1"
            class="absolute left-4 top-1/2 transform -translate-y-1/2 bg-white text-teal-600 p-3 rounded-full shadow-md hover:bg-gray-100 z-30" aria-label="Previous slide">
            <i class="fas fa-chevron-left"></i>
        </button>
        <button @click="current = (current === images.length - 1) ? 0 : current + 1"
            class="absolute right-4 top-1/2 transform -translate-y-1/2 bg-white text-teal-600 p-3 rounded-full shadow-md hover:bg-gray-100 z-30" aria-label="Next slide">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
            <div class="space-y-6">
                ...
                <p class="text-gray-700 text-lg leading-relaxed">
                    Welcome to <span class="font-semibold text-teal-600">MEDIBOOK AI</span>, where cutting-edge technology meets compassionate care.
                    Our mission is to revolutionize healthcare in Lebanon — combining expertise, empathy, and innovation to serve you better.
                </p>
                <p class="text-gray-600 text-base">
                    From AI-powered diagnostics to seamless appointment scheduling, we ensure every patient experience is efficient, personalized, and stress-free.
                    With a network of skilled doctors and modern clinics, we're redefining what it means to feel truly cared for.
                </p>
            </div>
    ...
    <section id="clinics" class="bg-gray-50 py-24">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="mb-8 max-w-2xl mx-auto">
                <input type="text" id="clinic-search" class="w-full px-5 py-3 rounded-lg border border-gray-300 shadow-sm focus:ring-2 focus:ring-teal-500 focus:outline-none" placeholder="🔍 Search clinics by name…" autocomplete="off" />
            </div>
            <div id="clinics-list" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10">
                @foreach ($clinics as $clinic)
                    <div class="bg-white rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 p-6 flex flex-col">
                        @if ($clinic->image)
                            <img src="{{ asset('storage/' . $clinic->image) }}" class="w-full h-48 object-cover rounded-md mb-4">
                        @endif
                        ...
                        <a href="{{ route('clinic.show', $clinic) }}" class="mt-4 inline-block bg-teal-500 hover:bg-teal-600 text-white font-semibold text-sm px-5 py-2 rounded-full transition text-center">View Doctors</a>
                    </div>
                @endforeach
            </div>
    ...
    <section id="doctors" class="bg-white py-24">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="mb-8 max-w-2xl mx-auto">
                <input type="text" id="doctor-search" class="w-full px-5 py-3 rounded-lg border border-gray-300 shadow-sm focus:ring-2 focus:ring-teal-500 focus:outline-none" placeholder="🔍 Search doctors by name…" autocomplete="off" />
            </div>
            <div id="doctors-list" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10">
                @foreach ($doctors as $doctor)
                    <div class="bg-gray-50 rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 p-6 flex flex-col">
                        @if ($doctor->image)
                            <img src="{{ asset('storage/' . $doctor->image) }}" class="w-full h-48 object-cover rounded-md mb-4">
                        @endif
                        <h3 class="text-xl font-semibold text-gray-800">Dr. {{ $doctor->first_name }} {{ $doctor->last_name }}</h3>
                        <p class="text-sm text-teal-600 mt-1">{{ $doctor->specialization }}</p>
                        <a href="{{ route('patient.appointment.create', $doctor) }}" class="mt-4 inline-block bg-teal-500 hover:bg-teal-600 text-white font-semibold text-sm px-5 py-2 rounded-full transition text-center">Book Appointment</a>
                    </div>
                @endforeach
            </div>
